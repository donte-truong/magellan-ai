...
The **Next.js frontend** (`frontend/`) provides a supply-chain workspace at **http://localhost:3000** (also available at `/research`): enter a product, explore its interactive node graph, inspect the evidence behind each relationship, and review its bill of materials. Without containers, start the backend on port 8000, then run `bun install` and `bun run dev` in `frontend/`.
...
The **landing page at http://localhost:3000/home** introduces Magellan with scroll-triggered iPhone decomposition and an interactive 3D globe. The curated **iPhone 17 Pro demo at http://localhost:3000/demo** follows product input → animated decomposition → supplier network. It uses sourced data, labels headquarters and illustrative routes, and works without model access. Older `/#network` links forward to `/demo#network`.
