...
`Run.provider` identifies `curated_fixture`, `tavily_openai`, or `tavily_openrouter`; this is independent of `mode: live | replay`. A fixture run is explicitly identified as cached public-source research. Replay jobs reconstruct the cached graph's node/edge events into a separate graph and flag every event with `mode: replay`.
