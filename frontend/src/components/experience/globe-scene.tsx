...
export default function GlobeScene() {
  ...
  useEffect(() => {
    ...
    scene.add(earth);
    earth.rotation.set(0.19, -0.6, -0.1);
    ...
    const move = (event: PointerEvent) => {
      ...
      target.x = THREE.MathUtils.clamp(target.x + (event.clientY - drag.y) * 0.003, -0.8, 0.8);
      drag = { x: event.clientX, y: event.clientY };
    };
    const up = () => {
      drag = null;
    ...
    const render = (time: number) => {
      ...
      const snapshot = `${state.supplier}:${width}:${height}:${target.x}:${target.y}:${control.current.zoom}:${control.current.reset}:${mapVersion}`;
      ...
      if (!drag && motion && !state.supplier) target.y += dt * 0.028;
      const damping = media.matches || state.paused ? 1 : 1 - Math.exp(-dt * 4);
      ...
      markers.forEach((entry, i) => {
        ...
        if (pin) {
          ...
          pin.style.transform = `translate(${(projected.x * 0.5 + 0.5) * width}px,${(-projected.y * 0.5 + 0.5) * height}px)`;
      ...
      {!failed && (
        <div className="globe-controls" aria-label="Globe controls">
          ...
          <button
            ...
            onClick={() => {
              useDemo.getState().selectSupplier(null);
