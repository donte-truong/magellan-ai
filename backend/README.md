...
4. **Resolves conservatively.** Entities match by normalized label within a kind, by an evidence-backed alias, or by part number only together with the same manufacturer and kind. Two matches, or a name with a conflicting identifier, emit `entity.review_needed` and never merge. Imported `bom_items` hints sharpen queries but are never used for matching.
