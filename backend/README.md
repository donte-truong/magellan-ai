...
Follow-ups, scenarios, and hypotheticals: `POST /graphs/{id}/research` starts a run bound to an existing graph under a natural-language instruction ("separate wafer fabrication from packaging and test for the A17 Pro as distinct steps with their own plants"); the instruction steers the planner and tells the extractor what to look for, only the target nodes and what the run creates under them are researched, and every evidence rule still applies. `POST /graphs/{id}/scenarios` forks a base graph into a scenario (mode `scenario`, with `parent_graph_id`), on which every endpoint works, follow-up research stays sandboxed, and `POST /graphs/{scenario}/edits` applies a hypothetical in natural language ("replace TSMC with Samsung Foundry", "remove the Zhengzhou plant"): the agent proposes structured operations, added nodes and edges are `user_asserted` with the instruction as their source and span under `data.custom.hypothetical`, and the base graph never changes. `POST .../scenarios/{sid}/reset` reverts the scenario to the base's latest real version; `DELETE` discards it.
