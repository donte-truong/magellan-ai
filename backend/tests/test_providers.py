...
async def test_live_provider_fetches_raw_text_and_checks_quotes_and_entailment():
    calls = []
    ...
    def handle(request):
        ...
        if data["text"]["format"]["name"] == "extraction":
...
async def test_location_verification_does_not_invent_coordinates():
    def handle(request):
        data = json.loads(request.content)
        if request.url.host == "api.tavily.com":
            return httpx.Response(
        ...
        if data["text"]["format"]["name"] == "locationextraction":
            return httpx.Response(
                200,
                json=response_data(
                    {
