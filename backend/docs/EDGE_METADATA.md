...
Implementation: `app/edge_metadata.py` (ported from the Node backend); contract: `docs/openapi.yaml` (`Edge`, `EdgeSource`, `EdgeConfidenceDetails`); tests: `tests/test_edge_metadata.py` plus graph/API coverage. Rejected claims are excluded from evidence; disputed claims count as contradiction.
